...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('presentation-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            ...
            const savedQuiz = localStorage.getItem('presentation-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('presentation-progress', JSON.stringify(this.progress));
        localStorage.setItem('presentation-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>プレゼン Academy へようこそ</h2>' +
            '<p>伝わるスライド設計と話し方の技術を体系的に学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('presentation-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('presentation-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            ...
            localStorage.removeItem('presentation-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
